...
EXTRACTION_SYSTEM = """You extract monitorable project obligations from Romanian
...
(kind=selected_option). Table rows preserve row/column relationships.
selected_option candidates have already been filtered deterministically so only
the option explicitly marked Selectată: Da remains.

Extract actual future/ongoing obligations: project indicators and targets,
implementation milestones, payment/reimbursement/procurement commitments,
selected scoring commitments that must remain true during
implementation/monitoring, durability/maintenance commitments, and explicit
beneficiary duties.

IMPORTANT EXCLUSIONS:
- Never extract an option marked Selectată: Nu.
- A selected scoring option is NOT automatically an obligation.
  Historical/application-time financial measurements (for example solvency/RSG,
  prior-year turnover ratios or past balance-sheet facts) are evaluation facts,
  not future monitoring obligations.
- Do not extract generic scoring formulas/rules such as how points are awarded.
- Do not extract isolated dates/date ranges, table headers, partial cells or
  fragments without the action/target they belong to.
- Do not extract purely historical descriptions, generic market analysis,
  optional rights, recommendations or speculative forecasts.
- For binary Da/Nu commitments, select the complete subcriterion statement, not
  the isolated word 'Da'.
- Prefer the most complete row/clause that contains action/target + quantity/value
  + deadline when present.

CRITICAL GROUNDING RULE: never translate, paraphrase, invent or repair source
wording. Return only the candidate_id plus the smallest contiguous
unit_start/unit_end range that contains the complete obligation.
...
If a deadline is relative to an event whose absolute date is not supplied in that
...
CRITICAL GROUNDING RULE: never invent or copy free-form quotations. Evidence must
be returned only as candidate_id + contiguous unit_start/unit_end pointers. The
application slices exact source text locally. A factual outcome must have
evidence. insufficient_evidence may have an empty evidence list.
        ...
        if not chunks:
            return [
                ValidationCandidate(
                    ...
                    rationale=(
                        "Documentele selectate nu conțin text extractabil suficient "
                        "pentru verificare."
                    ),
                    source_anchors=(),
                )
